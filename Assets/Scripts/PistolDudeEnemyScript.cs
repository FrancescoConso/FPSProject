...
public class PistolDudeEnemyScript : AbstractEnemy
{
    ...
    void Update()
    {
        ...
        {if(chaseTargetObj != null)
            {
                agent.destination = chaseTargetObj.transform.position + chaseTargetObj.transform.right * Random.Range(0, 7f) + chaseTargetObj.transform.forward * Random.Range(0, 7f);
                RaycastHit hitTarget;
                Physics.Linecast(transform.position, chaseTargetObj.transform.position, out hitTarget);
                if (agent.remainingDistance < 20f && hitTarget.transform != null && hitTarget.transform.gameObject.CompareTag("Player"))
                {
    ...
    {
        if (instantiatedGameObject != null) Instantiate(instantiatedGameObject, transform.position + transform.right - transform.up * 0.75f + transform.forward * 0.25f, Quaternion.identity);
